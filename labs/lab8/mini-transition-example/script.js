// data we work with
let data1 = [1, 2, 3];
let data2 = [180, 140, 110, 105];
let data3 = [1030, 1720, 1400];
...
let yMax = d3.max(data, function(d){
  ...
  // in the case of our data, there is only one value per data point
  // as opposed to an object with more complicated structure
  return d;
});
...
let enteringGroups = theSituation.enter()
  .append("g")
    ...
    .attr("transform", function(d, i){
      ...
      let yPos = yScale(d); //our yScale expects the data points value as an input
...
function update(newdata){
  ...
  // update yscale
  yMax = d3.max(data, function(d){ return d;}); // detailed description above
  ...
  enteringGroups.attr("class", "datapoint")
    ...
    .attr("transform", function(d, i){
      return "translate("+xScale(i)+","+yScale(d)+")"; // position AFTER transition
    })
  ;
